...
export default function TierCard({
  ...
  const isMostPopular = tier === "Tier 2";

  return (
    <div
      className={`relative flex flex-col rounded-2xl border p-6 lg:p-8 transition-all duration-500 ${
        isMostPopular
    ...
    >
      ...
      {/* Tier Header */}
      <div className={`mb-3 flex items-center gap-3 ${isMostPopular ? "mt-4" : ""}`}>
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-xl text-[#c1c1c1] ${
            isMostPopular ? "bg-[#c1c1c1]/15" : "bg-white/5"
          }`}
        >
          <Icon />
        </div>

        <p className="text-xs font-semibold tracking-[0.2em] uppercase text-[#c1c1c1]">
          {tier}
        </p>
      </div>
      ...
      <div
        className={`mb-4 h-px ${
          isMostPopular ? "bg-[#c1c1c1]/25" : "bg-white/10"
        }`}
      />
      ...
      {extraServices?.length > 0 && (
        <button
          ...
          className="mt-3 flex items-center gap-1.5 self-start rounded-lg px-2 py-1 text-xs font-medium text-[#c1c1c1] hover:bg-white/5 transition-colors"
        >
          <span>
            {showMore ? "Show less" : `Show ${extraServices.length} more`}
          </span>
      ...
      {isMostPopular && (
        <div className="mt-5 rounded-xl border border-[#c1c1c1]/20 bg-[#c1c1c1]/5 p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-semibold tracking-[0.15em] uppercase text-[#c1c1c1]">
              + Includes
            </p>

            <p className="text-sm font-medium text-white/75">
              Full Interior Reset Package
            </p>
          </div>
        </div>
      )}
