...
const StepIcon = ({ type }) => {
  ...
  const icons = {
    booking: <FaCalendarCheck size={32} color="#e80202" />,
    planning: <FaClipboardList size={32} color="#e80202" />,
    detailing: <FaCar size={32} color="#e80202" />,
    advice: <FaCheckCircle size={32} color="#e80202" />,
    // Fallbacks from previous data types
    local: <FaHome size={32} color="#e80202" />,
  };

  return <>{icons[type] || <FaHome size={32} color="#e80202" />}</>;
};
    ...
    <section className="py-24 bg-[#000000]">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-5xl mx-auto relative">
          {/* Connector line */}
          <div className="hidden md:block absolute top-12 left-0 right-0 h-0.5 bg-[linear-gradient(to_right,#e8020222,#e80202,#e8020222)]" /> {/* Accent color for connector line */}
          ...
          <div className="grid md:grid-cols-4 gap-10 relative">
            {data.steps.map((step) => (
              ...
              >
                <div className="relative z-10 w-24 h-24 rounded-full bg-[#e80202]/10 flex items-center justify-center mb-6 shadow-md hover:bg-[#e80202] transition-all duration-300">
                  <StepIcon type={step.iconType} />
                </div>
                <div className="space-y-2">
                  <div className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#c1c1c1] text-[#e80202] font-bold text-sm mb-2">
                    {step.id}
                  </div>
          ...
          <p className="text-base md:text-lg text-[#ffffff] leading-relaxed"> {/* White text for footer */}
            {data.footerText.includes("20–40 minutes") ? (
              <>
                ...
                <span className="font-semibold text-[#e80202]"> 20–40 minutes </span> {/* Accent color for highlighted text */}
